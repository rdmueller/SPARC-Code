...
build_claude_flags() {
    ...
    # Nur --mcp-config anhängen, wenn die Datei existiert
    if [[ -f "$MCP_CONFIG" ]]; then
        flags="--mcp-config $MCP_CONFIG $flags"
    fi
...
main() {
    ...
    # Prüfe, ob das Kommando 'claude' verfügbar ist, installiere es ggf.
    if ! command -v claude >/dev/null 2>&1; then
        echo "'claude' nicht gefunden. Installiere @anthropic-ai/claude-code global via npm..."
        npm install -g @anthropic-ai/claude-code
        # Prüfe erneut
        if ! command -v claude >/dev/null 2>&1; then
            echo "Fehler: 'claude' konnte nicht installiert werden." >&2
            exit 1
        fi
    fi
